...
(define-test contfracs 
  (assert-equalp (rat->contfrac 33/137) '(0 4 6 1 1 2))
  (assert-equalp (rat->contfrac 137/33) '(4 6 1 1 2))
  (assert-equalp (convergents (rat->contfrac 137/33))
		 '(4 25/6 29/7 54/13 137/33))
  (assert-equalp (convergents (rat->contfrac 33/137))
...
(define-test integer-digits
  ...
  (assert-equalp (integer->digit-list 12345 :base 10) '(1 2 3 4 5))
  (assert-equalp (integer->digit-list 0 :base 10) '(0))
  (assert-equalp (integer->digit-vector 0 :base 10) #(0)) 
  (assert-equalp (integer->digit-vector 1234 :base 10) #(1 2 3 4))
  (assert-equalp (integer->digit-vector 1234832840324 :base 311) 
		 #(131 310 95 286 102))
  (assert-equal (digit-seq->integer #(0) :base 10) 0)
  (assert-equal (digit-seq->integer #(1234)) 1234)
  (assert-equal (digit-seq->integer #(1 1 1) :base 2) 7)
  (assert-equal (digit-seq->integer '(1 2 3 4) :base 10) 1234)
  )
